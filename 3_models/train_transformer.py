#!/usr/bin/env python3
"""
Trains and profiles the Flow-Transformer sequence classifier with [CLS] Token Attention.
Includes:
- Multi-Head Self-Attention on raw packet length/direction sequences
...
from architectures import WebTunnelTransformer
from utils import (
    FlowSequenceDataset,
    BinaryFocalLoss,
    load_sequence_data,
...
def train_transformer(
    ...
    train_ds = FlowSequenceDataset(X_train, y_train, channel_first=False)
    val_ds = FlowSequenceDataset(X_val, y_val, channel_first=False)
    test_ds = FlowSequenceDataset(X_test, y_test, channel_first=False)

    train_loader = DataLoader(train_ds, batch_size=batch_size, shuffle=True)
    val_loader = DataLoader(val_ds, batch_size=batch_size, shuffle=False)
    test_loader = DataLoader(test_ds, batch_size=batch_size, shuffle=False)

    model = WebTunnelTransformer(
        in_features=2,
        d_model=64,
        nhead=4,
        num_layers=2,
    ...
    print(f"\n[OK] Model saved to {TRANSFORMER_MODEL_PATH}")
    res_path = os.path.join(EVALUATION_DIR, "transformer_results.json")
    ...
    np.savez_compressed(
        os.path.join(EVALUATION_DIR, "transformer_test_preds.npz"),
        probs=test_probs,
        y_test=y_test
    )
