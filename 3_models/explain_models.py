...
def explain_xgboost():
    ...
    # 1. Feature Importances (Information Gain)
    ...
    plt.title("XGBoost Feature Importance (Information Gain)", fontsize=13, fontweight="bold")
    ...
    try:
        ...
        plt.figure(figsize=(10, 6))
        shap.summary_plot(shap_values, df_test, max_display=12, show=False)
        plt.title("SHAP Feature Attribution: Impact on WebTunnel Classification", fontsize=13, fontweight="bold", pad=15)
        plt.tight_layout()
        plt.savefig(os.path.join(PLOT_DIR, "xgboost_shap_summary.png"), dpi=300, bbox_inches='tight')
        plt.close()
        print(f"[OK] Saved {PLOT_DIR}/xgboost_shap_summary.png")
...
def explain_1d_cnn_saliency():
    ...
    plt.title("1D-CNN Saliency Map: Which Packets Trigger WebTunnel Detection?", fontsize=13, fontweight="bold")
...
def explain_transformer_attention():
    ...
    plt.plot(packet_indices, mean_attention, color="#d62728", linewidth=2.0, label="Transformer Token Gradient Saliency")
    plt.fill_between(packet_indices, 0, mean_attention, alpha=0.3, color="#d62728")
    plt.title("Flow-Transformer Token Attribution: Input Gradient Saliency Across Sequence", fontsize=13, fontweight="bold")
    plt.xlabel("Packet Token Index in Flow (Packets 1-200)", fontsize=12)
    plt.ylabel("Gradient Attribution Weight", fontsize=12)
